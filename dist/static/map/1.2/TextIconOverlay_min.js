...
(function () {
     ...
     */
    var _IMAGE_PATH = 'http://api.map.baidu.com/library/TextIconOverlay/1.2/src/images/m0';
    //var _IMAGE_PATH = '../../../../imgs/common/index_project';
    ...
    TextIconOverlay.prototype._setupDefaultStyles = function(){
        ...
        this._styles.push({
            ...
            size: new BMap.Size(53, 53)//设置地图聚合图片的大小
        });
    ...
    */
    TextIconOverlay.prototype.setText = function(text){
        ...
        //自定义聚合样式文字 判断
        //if(text && (!this._text || (this._text.value != text.value) || (this._text.name != text.name)|| (this._text.name1 != text.name1))){
            this._text = text;
            this._updateText();
            this._updateCss();
            this._updatePosition();
    ...
    */
    TextIconOverlay.prototype._updateText = function(){
        if (this._domElement) {
            ...
            //var name = (this._text.name.length>3) ? this._text.name.substring(0,2) : this._text.name;//截取前两个
            //this._domElement.innerHTML = '<p class="map_p">'+this._text.value+'</p>'+'<p title=' +'"'+this._text.name +'">'+name+'</p>'/*+'<p>'+this._text.name1+'</p>'*/;
        }
    };
